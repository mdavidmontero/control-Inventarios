...
export const TableInventariosPuntos = ({ data }) => {
  const deleteInventarioPunto = useInventariosPuntosStore(
    (state) => state.deleteInventarioPunto
  );
  ...
  const removeBrand = (p) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        await deleteInventarioPunto({ id: p.id });
      }
    });
  };
  ...
  const columns = [
    ...
    {
      accessorKey: "nombre_punto",
      header: "Punto",
      cell: (info) => (
        <td data-title="Punto" className="ContentCell">
          <span>{info.getValue()}</span>
        </td>
      ),
    },
